...
def clustering(allocation=normal_cluster):
    ...
    for iter in range(100):
        rand_X = np.random.choice(X, 100, replace=False)
        gf = gaussian(mu, sigma)
        gamma = estimate_posterior_likelihood(rand_X, pi, gf)
        update_gamma = allocation(gamma)
        mu, sigma, pi = estimate_gmm_parameter(rand_X, update_gamma)
        # piの更新時にNを割っているが、これだと1を超えてしまうのでアウト
        print(pi)
        gf = gaussian(mu, sigma)
        log_likelihood = calc_log_likelihood(rand_X, pi, gf)
